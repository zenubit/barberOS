...
export default function DashboardView() {
  const [stats, setStats] = useState({ today: 0, pending: 0, barbers: 0, services: 0 });
  ...
  useEffect(() => {
    (async () => {
      ...
      setStats({ today: todayAppts?.length || 0, pending: pending?.length || 0, barbers: barbers?.length || 0, services: services?.length || 0 });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <KPI label="Citas hoy" value={stats.today} icon="Calendar" />
      ...
      <h3 className="font-display font-semibold mb-4">Agenda de hoy</h3>
      <div className="surface overflow-hidden overflow-x-auto">
        <table className="table">
          <thead><tr><th>Hora</th><th>Cliente</th><th>Servicio</th><th>Trabajador</th><th>Estado</th></tr></thead>
          <tbody>
            {todayList.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8" style={{ color: 'var(--ink-faint)' }}>Sin citas hoy.</td></tr>
            )}
            {todayList.map(a => (
              <tr key={a.id}>
